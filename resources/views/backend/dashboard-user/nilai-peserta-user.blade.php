    ...
    <body>
        ...
        <script>
            document.addEventListener("DOMContentLoaded", function () {
                fetch("{{ route('user.tanggal-test') }}")
                    .then(response => response.json())
                    .then(data => {
                        let select = document.getElementById("testDateSelect");
                        select.innerHTML = '<option value="">Pilih tanggal test...</option>'; // Reset option pertama
                    ...
                    .catch(error => console.error("Error fetching test dates:", error));
            });
        </script>
        <script>
            document.addEventListener("DOMContentLoaded", function () {
                let testDateSelect = document.getElementById("testDateSelectt");
                let selectedTestDate = document.getElementById("selectedTestDatee");

                // Ambil tanggal dari backend
                fetch("{{ route('user.tanggal-test') }}")
                    .then(response => response.json())
                    .then(data => {
                        testDateSelect.innerHTML = '<option value="">Pilih tanggal test...</option>';
                        if (data.length > 0) {
                            data.forEach(date => {
                                let option = document.createElement("option");
                                option.value = date.split(" ")[0]; // Hanya ambil YYYY-MM-DD
                                option.textContent = new Date(date).toLocaleDateString('id-ID');
                                testDateSelect.appendChild(option);
                            });
                        } else {
                            testDateSelect.innerHTML = '<option value="">Tidak ada data tersedia</option>';
                        }
                    })
                    .catch(error => console.error("Error fetching test dates:", error));

                // Update input hidden saat pengguna memilih tanggal
                testDateSelect.addEventListener("change", function () {
                    selectedTestDate.value = this.value;
                    console.log("Tanggal test yang dikirim:", this.value); // Debug
                });
            });
        </script>
